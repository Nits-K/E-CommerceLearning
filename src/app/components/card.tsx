...
const Card=({id,brand,size,price,image})=>{
    return(
        
        <div className="bg-white border-white border-2 max-w-sm rounded overflow-hidden shadow-lg flex flex-col m-4">
          <div className="aspect-w-3 aspect-h-4">
  <img className=" w-full object-cover" src={`/images/${image}`} alt="T-shirts"/>
  </div>
  <div className="px-6 py-4">
    <div className="font-bold text-gray-600 font-serif text-xl mb-2">{brand}</div>
  </div>
  <div className="px-6 pt-4 pb-2">
    <span className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{size}</span>
